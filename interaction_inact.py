...
import choose_behavior_inact
import robotimer_inact
...
def m1(chbh_to_timer, event_end, bh_end, q, eventi):
    choose_behavior.choose_behavior(chbh_to_timer, event_end, bh_end, q, eventi)

def r1(q, event, event_end, eventface):
    recording(q, event, event_end, eventface)
...
if __name__ == '__main__':
    ...
    event_end = multiprocessing.Event()
    ...
    th0t = multiprocessing.Process(target=m1, args=(chbh_to_timer, event_end, bh_end, q, eventi))
    th0t.start()

    th2t = multiprocessing.Process(target=r1, args=(q, event, event_end, eventface))
    th2t.start()
    robotimer.roberttimer(chbh_to_timer, timer_to_images, timer_to_eyepos, event_end, bh_end)
